...
function activate (context) {
  ...
  let run = vscode.commands.registerCommand('extension.miramac.node.exec', function () {
    ...
    if (!config.terminalMode) {
      ...
      let terminalId = terminals.length
      terminals.push(vscode.window.createTerminal(`Node.js (${terminalId + 1})`))
      terminals[terminalId].show()
      terminals[terminalId].sendText(`node "${tmpFile}"`)
      setTimeout(function () { fs.unlink(tmpFile) }, 1000)
    }
  })
