  ...
  let run = vscode.commands.registerCommand('extension.miramac.node.exec', function () {
    ...
    if (!config.terminalMode) {
      ...
      process.on('close', function () {
        ...
        if (runningStatus) {
          if (config.showInfo) {
            outputChannel.appendLine('Info: End process in ' + getDuration(startTime, new Date()) + ' (mm:ss:fff)')
          }
      ...
      process.on('error', function (err) {
        ...
        if (config.showInfo) {
          outputChannel.appendLine('Info: End process with errors in ' + getDuration(startTime, new Date()) + ' (mm:ss:fff)')
        }
        if (runningStatus) {
          runningStatus.dispose()
          runningStatus = null
