...
function activate (context) {
  var vqProcess
  var outputChannel = vscode.window.createOutputChannel('NodeExec')
  ...
  var run = vscode.commands.registerCommand('extension.miramac.node.exec', function () {
    ...
    var tmpFile = path.join(dirName, `tmp_${require('crypto').createHash('sha1').update(Math.random().toString()).digest('hex').substr(0, 13)}.tmp`)
